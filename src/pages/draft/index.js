...
import { CardDraft } from 'src/components/card/draft'
...
import { fetchDraft } from 'src/api/axios/ost_api'
...
const Draft = () => {
  ...
  async function fetchData() {
    try {
      ...
      const { data } = await fetchDraft(payload)
      setDatas(data.posts)
  ...
  return (
    <>
      ...
      <Grid container spacing={6}>
        {datas?.map(data => (
          <CardDraft key={data.id} data={data} callbackReload={callbackReload} />
        ))}
      </Grid>
      <Pagination
        sx={{ mt: 10 }}
